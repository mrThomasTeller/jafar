...
if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || '')) {
  process.on('exit', function() {
    debug(queue)
    __webpack_require__(7).equal(queue.length, 0)
  })
}
...
const mkdirs = u(__webpack_require__(40))
const mkdirsSync = __webpack_require__(41)
...
const u = __webpack_require__(2).fromCallback
const rimraf = __webpack_require__(44)
...
module.exports = {
  copySync: __webpack_require__(43)
}
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_glob_promise__ = __webpack_require__(24);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_glob_promise___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_glob_promise__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_fs_extra__ = __webpack_require__(33);
...
const work = async (state, replace) => {
    const { from, to, where, regex } = state;
    ...
    let oldContent = {};
    let processed = 0;
    const files = await __WEBPACK_IMPORTED_MODULE_0_glob_promise__(__WEBPACK_IMPORTED_MODULE_2_path__["join"](where, '**/*'), { nodir: true });

    try {
        for (let file of files) {
            const content = (await __WEBPACK_IMPORTED_MODULE_1_fs_extra__["readFile"](file)).toString();
            oldContent = Object(__WEBPACK_IMPORTED_MODULE_3_lodash__["merge"])({}, oldContent, { [file]: content });
            const needle = new RegExp(regex ? from : Object(__WEBPACK_IMPORTED_MODULE_3_lodash__["escapeRegExp"])(from), 'g');
            content.replace(needle, content => {
                matches = matches.concat({ file: __WEBPACK_IMPORTED_MODULE_2_path__["relative"](where, file), content });
                return '';
            });
            setState({ matches, oldContent });
            const curReplacesCount = content.split(needle).length - 1;

            const newContent = content.replace(needle, to);
            if (curReplacesCount > 0) {
                occurrencesCount += curReplacesCount;
                ++filesCount;
                setState({ occurrencesCount, filesCount });
                if (replace) {
                    await __WEBPACK_IMPORTED_MODULE_1_fs_extra__["writeFile"](file, newContent);
                }
            }

            ++processed;
            setState({ progress: processed / files.length });
...
const setState = (state, end = false) => {
    postMessage(JSON.stringify({ state, end }));
};
...
const glob = __webpack_require__(12)
...
jsonFile.outputJsonSync = __webpack_require__(48)
// aliases
...
const file = __webpack_require__(53)
const link = __webpack_require__(54)
const symlink = __webpack_require__(55)
...
const _symlinkPaths = __webpack_require__(56)
const symlinkPaths = _symlinkPaths.symlinkPaths
const symlinkPathsSync = _symlinkPaths.symlinkPathsSync

const _symlinkType = __webpack_require__(57)
...
/***/ })
/******/ ]);
//# sourceMappingURL=67d92b597a2e2728815e.worker.js.map